...
io.on("connection", (socket) => {
  ...
  socket.on("drawing-action", (data) => {
    console.log(`🎨 Drawing action from ${data.userName} in room ${data.roomId}`);
    // Broadcast to all other users in the room
    socket.to(data.roomId).emit("drawing-action", data);
  });
  ...
  // ==========================
  socket.on("ai-report-generated", ({ roomId, report, userName }) => {
    console.log(`📋 AI report generated by ${userName} in room ${roomId}`);
    // Broadcast to all other users in the room
    socket.to(roomId).emit("ai-report-update", {
      report: report,
      userName: userName
    });
  });
